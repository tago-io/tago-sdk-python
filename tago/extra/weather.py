...
class Weather:
    ...
    def current(self, query, full, lang):
        ...
        url = 'http://api.wunderground.com/api/'+self.key+'/lang:'+self._lang + \
            '/conditions/q/'+urllib.quote(self._query.encode("utf-8"))+'.json'
        method = 'GET'
    ...
    def history(self, date, query, full, lang):
        ...
        try:
            ...
            date = str(date.year) + str(format(date.month, '02')) + \
                str(format(date.day, '02'))
        except Exception as e:
            return Promise.reject('Invalid date', + e)
        url = 'http://api.wunderground.com/api/'+self.key+'/lang:'+self._lang + \
            '/history_'+str(date)+'/q/' + \
            urllib.quote(self._query.encode("utf-8"))+'.json'
        method = 'GET'
        self.default_options['url'] = url
        self.default_options['method'] = method
        self.default_options['params'] = params
    ...
    def forecast(self, query, full, lang):
        ...
        url = 'http://api.wunderground.com/api/' + self.key + \
            '/forecast/q/'+urllib.quote(self._query.encode("utf-8"))+'.json'
        method = 'GET'
        self.default_options['url'] = url
        self.default_options['method'] = method
        self.default_options['params'] = params
    ...
    def alerts(self, query, full, lang):
        ...
        params = {'query': query, 'full': full, 'lang': lang}
        self._setParams(params)
        url = 'http://api.wunderground.com/api/'+self.key+'/alerts/q/' + \
            urllib.quote(self._query.encode("utf-8"))+'.json'
        method = 'GET'
        self.default_options['url'] = url
        self.default_options['method'] = method
