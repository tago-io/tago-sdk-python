...
def invite(type, ref_id, data, default_options):
  ...
  return requests.post('{api_endpoint}/share/{type}/{ref_id}'.format(api_endpoint=API_TAGO, type=type, ref_id=ref_id), headers=default_options, data=data).json()

def edit(share_id, data, default_options):
  data = data if data else {}
  if share_id is None or share_id == '':
    raise ValueError('share_id must be set')
  return requests.put('{api_endpoint}/share/{share_id}'.format(api_endpoint=API_TAGO, share_id=share_id), headers=default_options, data=data).json()
