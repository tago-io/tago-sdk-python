...
def test_email():
    with requests_mock.Mocker() as cur_mock:
        ...
        # Good case
        result = email(TOKEN).send('xyz@ncsu.edu', 'tago test',
                                   'test tago services', 'xyz@ncsu.edu', 'q')
        check_status(result, True)

        # Bad case
        result = email(TOKEN).send('wrong_email', 'tago test',
                                   'test tago services', 'xyz@ncsu.edu', 'q')
        check_status(result, False)

        # Bad case
        result = email('wrong_token').send(
            'xyz@ncsu.edu', 'tago test', 'test tago services', 'xyz@ncsu.edu', 'q')
        check_status(result, False)
